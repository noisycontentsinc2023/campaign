    ...
    async def roll_the_dice(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if cell:
            ...
            if dice_count > 0:
                ...
                await interaction.followup.send(f'{interaction.user.mention}이 {self.ctx.author.mention}의 주사위를 굴려 {dice_roll} 가 나왔습니다! 새로운 위치: {new_location_name}', ephemeral=True)
